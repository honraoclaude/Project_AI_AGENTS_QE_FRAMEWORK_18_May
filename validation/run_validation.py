...
Usage:
    ...
    python -m validation.run_validation --skip-report       # JSON only, no HTML report
    python -m validation.run_validation --skip-dashboard    # skip dashboard regeneration
...
async def _run_story(story_id: str, skip_report: bool) -> None:
...
async def main(story_id: str, skip_report: bool, skip_dashboard: bool = False, all_stories: bool = False) -> None:
    if all_stories:
        for sid in STORIES:
            await _run_story(sid, skip_report)
    else:
        await _run_story(story_id, skip_report)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-dashboard", action="store_true",
                        help="Skip dashboard regeneration (dashboard is generated by default)")
    parser.add_argument("--all-stories", action="store_true",
                        help="Run validation for all stories in STORIES registry")
    args = parser.parse_args()
    asyncio.run(main(args.story, args.skip_report, args.skip_dashboard, args.all_stories))
